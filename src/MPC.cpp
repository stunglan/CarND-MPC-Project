...
// the majority of this code is from the quiezzes
using CppAD::AD;
...
public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int t = 1; t < N; t++) {
      ...
      //      Changed for higher order polynom / code from slack / otherwise my car drives off the road
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  for (int i = 0;i < result.size();i++) {
    cout  << " " << std::setprecision(5) << result[i];
  }
  cout   << std::endl;
   */
